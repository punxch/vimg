...
function M:peek(job)
	...
	local cha_manifest = fs.cha(Url(tostring(cache_avif) .. ".json"))
	if cha_avif and cha_avif.len > 0 and cha_manifest and cha_manifest.len > 0 then
	...
	-- Generate avif synchronously (blocks ~2.5s, static already shown)
	self:generate_avif(job, cache)

	-- Refresh with avif if generated
	local cha2 = fs.cha(cache_avif)
	local manifest2 = fs.cha(Url(tostring(cache_avif) .. ".json"))
	if cha2 and cha2.len > 0 and manifest2 and manifest2.len > 0 then
