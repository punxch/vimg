...
pub(super) fn availability(plan: &CapturePlan) -> anyhow::Result<()> {
        ...
        plan.capture_frames() == 30,
        "Capture backend libav is unavailable: requires the fixed Preview profile (30 frames)"
    );
    ensure!(
        plan.frame_dimensions().1 == 160,
        "Capture backend libav is unavailable: requires the fixed Preview profile (height 160)"
    );
    ensure!(
