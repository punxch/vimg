...
- [x] A Capture plan computes shared media properties, capture windows, frame schedule, output dimensions, and labels once.
...
The plan now includes the selected stream's source time base and materializes one `FrameSchedule` per capture window. The Yazi descriptor and `vimg send` protocol carry a validated positive time-base numerator/denominator for the unambiguous single-video-stream case, so that complete preview descriptor skips the redundant probe; multi-video or incomplete descriptors retain the existing ffprobe fallback. The plan test verifies all nine schedules are initialized with the shared `1/1000` time base and 30-frame target.

The representative module stream test consumes all 270 observable frames and confirms complete authority records. The release default command remains byte-identical (`406db119839f6307cf56b5907966f525f36f18a21093f439926835918ab2c68f`), with 270 PTS from `183100` to `3113736`. Paired same-machine measurements were 1.64s for the refactor versus 1.56–1.72s for the Ticket 02 baseline. JPG and custom-filter compatibility runs pass; WebP retains the baseline's existing Broken pipe failure. Full tests and Clippy pass.
